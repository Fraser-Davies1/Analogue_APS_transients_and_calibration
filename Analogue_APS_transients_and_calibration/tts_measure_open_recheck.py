...
def capture_tts_ramp(bench, label, duration_s=5.0):
    print(f"\n>>> INITIATING CAPTURE (NO BLU-TACK): {label}")
    sys.stdout.flush()
    
    # Increase scale slightly if ambient light is high (50mV/div)
...
def run_open_recheck():
    ...
    try:
        with Bench.open("config/bench.yaml") as bench:
            t, v, slope, r2 = capture_tts_ramp(bench, "AMBIENT_RECHECK")
            i_derived = slope * C_integrator
            
            print(f"\n>>> RESULTS (OPEN/RECHECK)")
            print(f"    Derived Current: {i_derived*1e9:.3f} nA")
