...
def run_final_report_transient():
    print("--- Generating Final Report Transients (10% to 90%) ---")
    ...
    with Bench.open("bench.yaml") as bench:
        ...
        for pct, v_led in final_v_list:
            ...
            if not ref_plotted:
                plt.plot(time_ms, reset_v, 'k--', alpha=0.3, label="Reference Reset (CH3)")
                ref_plotted = True

        plt.xlabel("Time (ms)")
        plt.ylabel("Voltage (V)")
        plt.title("Pixel Transient Response: Linear Light Intensity Sweep (10% to 90%)\nVDD=5V, Reset=5V, 500Hz, 2 Cycles")
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("pixel_final_report_plot.png")
        print("\n--- Final Report Complete ---")
        print("Final report plot saved to pixel_final_report_plot.png")

if __name__ == "__main__":
    run_final_report_transient()
