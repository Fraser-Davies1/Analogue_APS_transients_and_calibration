...
def run_ultimate_dark_current_tts():
    ...
    print("   REF: CH2 (Reset) | SENSOR: CH1                   ")
    print("====================================================\n")
    ...
    try:
        with Bench.open("config/bench.yaml") as bench:
            ...
            # SigGen CH1: Reset Signal (5V Square, High-Z)
            ...
            # Trigger on Reset Falling Edge (CH2)
            ...
            for t_target in integration_times:
                ...
                # Set window to capture 2 cycles
                ...
                # Data Extraction (CH1: Sensor, CH2: Reset)
                ...
                if len(falls) > 0:
                    ...
                    if len(r_pts) > 0:
                        ...
                        plt.axhline(v_sat_target, color='r', linestyle=':', label='70% Threshold')
                        plt.title(f"Dark Current Run {t_target}s")
                        plt.legend()
                        plt.grid(True, alpha=0.3)
                        plt.savefig(f"results/plots/dark_run_final_{t_target}s.png")
                        plt.close()
                    else:
                        print("    [!] Error: Next rising edge not found.")
                else:
                    print("    [!] Error: No falling edge found in CH2.")
