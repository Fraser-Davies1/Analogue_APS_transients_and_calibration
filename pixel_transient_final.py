...
def run_final_transient():
    print("--- Starting Final Linearized Transients (Including 95% and Ref Waveform) ---")
    
    # 1. Manually specify the voltages to ensure 95% is included accurately
    # Levels: 10, 20, 30, 50, 70, 90, 95%
    # Using the previously calculated LUT values + the 95% point
    lut = pl.read_csv("led_lin_lut.csv")
    test_points = lut.filter(pl.col("light_percent").is_in([10, 20, 30, 50, 70, 90]))
    
    # Construct final test list
    final_v_list = []
    for row in test_points.iter_rows(named=True):
        final_v_list.append((row['light_percent'], row['v_led_control']))
    final_v_list.append((95, 3.4000)) # Adding the 95% point
    ...
    with Bench.open("bench.yaml") as bench:
        ...
        for pct, v_led in final_v_list:
            ...
            # Plot Reference Waveform (CH3) once for context
            if not ref_plotted:
        ...
        plt.title("Final Pixel Transients (2 Cycles)\nLinearized Intensity Sweep (10% to 95%) with Reset Reference")
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("pixel_final_report_plot.png")
        print("\n--- Final Experiment Complete ---")
        print("Final plot saved to pixel_final_report_plot.png")
